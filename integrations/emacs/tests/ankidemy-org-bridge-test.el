...
(ert-deftest ankidemy-org-bridge-suspends-and-redacts-disallowed-active-root ()
  (let* ((allowed (file-name-as-directory
                   (make-temp-file "ankidemy-approved-root-" t)))
         (disallowed (file-name-as-directory
                      (make-temp-file "ankidemy-private-root-" t)))
         (org-roam-directory disallowed)
         (ankidemy-org-bridge-allowed-roots (list allowed))
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'cancel-timer)
                  ...
                  ((symbol-function 'run-at-time)
                   (lambda (&rest _args) (setq scheduled t) 'new-timer))
                  ((symbol-function 'run-with-idle-timer)
                   (lambda (&rest _args) (setq scheduled t) 'new-idle-timer))
          ...
          (should-not (ankidemy-org-bridge--active-allowed-root))
          ...
          (ankidemy-org-bridge--root-watcher
           'org-roam-directory disallowed 'set nil)
          (should (memq 'save-timer cancelled))
          (should (memq 'presence-timer cancelled))
          (should-not scheduled))
      (delete-directory allowed t)
      (delete-directory disallowed t))))

(ert-deftest ankidemy-org-bridge-auth-and-health-do-not-disclose-disallowed-root ()
  (let* ((allowed (file-name-as-directory
                   (make-temp-file "ankidemy-approved-root-" t)))
         (disallowed (file-name-as-directory
                      (make-temp-file "ankidemy-private-root-" t)))
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'ankidemy-org-bridge--json-send)
          ...
          (should (string= "authenticated" (alist-get 'type (car sent))))
