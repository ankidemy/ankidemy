.PHONY: up down restart build logs clean purge-volumes reset-db debug-up debug-server debug-client reset-db-confirm rebuild-server rebuild-client up-dev down-dev server-logs client-logs

# ---------------------------------------
# PRODUCTION OPERATIONS
# ---------------------------------------

# Start all services with BuildKit cache
up:
...
# Manual server restart for development (when code changes)
restart-server:
	docker compose -f docker-compose.yml -f docker-compose.dev.yml restart server

# Build and run server with output
build-run-server:
	@echo "Building and running server..."
	docker compose -f docker-compose.yml -f docker-compose.dev.yml exec server sh -c "cd /app && go build -o ./tmp/main . && ./tmp/main"
