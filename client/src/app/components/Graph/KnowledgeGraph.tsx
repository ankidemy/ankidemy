...
const KnowledgeGraphInner: React.FC<KnowledgeGraphProps> = ({
  ...
  const [_frenzyQuestAutoSaveStatus, setFrenzyQuestAutoSaveStatus] = useState<'idle' | 'saving' | 'saved' | 'error'>('idle');
  ...
  }, [nodeCreationPosition, getGraphCenter, computeSpawnPositionWithAdaptiveNoise, stableGraph.nodes]);
  ...
  const handleOptionsExportJson = useCallback(async () => {
  ...
  const handleToolbarExport = useCallback(async (format: 'json' | 'yaml' = 'yaml') => {
    ...
    try {
      ...
      const filenameParts = orderedExportedCodes
        .slice(0, 2)
        .map(code => sanitizeFilenamePart(nodeNameByCode.get(code) || code))
        .filter(Boolean);
      const fallbackBase = sanitizeFilenamePart(currentDomainName) || 'selected_nodes';
  ...
  const toolbarToggles = useMemo(() => ([
    {
      id: 'frenzy',
      label: 'Frenzy',
      title: isFrenzyEnabled ? 'Frenzy on' : 'Frenzy off',
      active: isFrenzyEnabled,
      icon: <Zap size={9} />,
      onClick: toggleFrenzyEnabled,
    },
