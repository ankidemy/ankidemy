...
// ListByDomain returns all meta exercises for a domain.
func (d *MetaExerciseDAO) ListByDomain(domainID uint) ([]models.MetaExercise, error) {
...
func (d *MetaExerciseDAO) Create(meta *models.MetaExercise, prerequisiteIDs []uint, weights map[uint]float64) error {
	return d.db.Transaction(func(tx *gorm.DB) error {
		...
		for _, pid := range prerequisiteIDs {
			var cnt int64
			if err := tx.Model(&models.Definition{}).Where("id = ?", pid).Count(&cnt).Error; err != nil {
				return err
			}
			if cnt == 0 {
				continue
...
func (d *MetaExerciseDAO) AddVersion(metaID uint, req *models.ExerciseVersionRequest) (*models.Exercise, error) {
	...
	if err := d.db.Create(ex).Error; err != nil {
		return nil, err
	}
	return ex, nil
}
...
func (d *MetaExerciseDAO) UpdateVersion(versionID uint, req *models.ExerciseVersionRequest) (*models.Exercise, error) {
	...
	if err := d.db.Save(&ex).Error; err != nil {
		return nil, err
	}
	return &ex, nil
}
...
func (d *MetaExerciseDAO) DeleteVersion(versionID uint) error {
	return d.db.Transaction(func(tx *gorm.DB) error {
		...
		var ex models.Exercise
		if err := tx.First(&ex, versionID).Error; err != nil {
			return err
		}
...
func (d *MetaExerciseDAO) FindByID(id uint) (*models.MetaExercise, []models.Exercise, error) {
	...
	if err := d.db.Where("meta_exercise_id = ?", id).Order("id ASC").Find(&versions).Error; err != nil {
		return nil, nil, err
	}
...
func (d *MetaExerciseDAO) FindByCodeAndDomain(code string, domainID uint) (*models.MetaExercise, []models.Exercise, error) {
	...
	if res.Error != nil {
		if errors.Is(res.Error, gorm.ErrRecordNotFound) {
			return nil, nil, errors.New("meta exercise not found")
		}
		return nil, nil, res.Error
	}
	var versions []models.Exercise
	if err := d.db.Where("meta_exercise_id = ?", meta.ID).Order("id ASC").Find(&versions).Error; err != nil {
		return nil, nil, err
	}
	return &meta, versions, nil
}
