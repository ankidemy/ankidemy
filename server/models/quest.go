...
// MetaQuest represents a quest definition node (schedule + visibility + position).
...
func (MetaQuest) TableName() string { return "meta_quests" }

// QuestVersion represents versioned quest content.
type QuestVersion struct {
	ID            uint            `gorm:"primaryKey" json:"id"`
	MetaQuestID   uint            `gorm:"column:meta_quest_id;not null;index" json:"metaQuestId"`
...
// UserMetaQuestState stores per-user state for a quest definition.
type UserMetaQuestState struct {
	ID                uint       `gorm:"primaryKey" json:"id"`
	UserID            uint       `gorm:"column:user_id;not null;index;uniqueIndex:idx_user_meta_quest" json:"userId"`
	MetaQuestID       uint       `gorm:"column:meta_quest_id;not null;index;uniqueIndex:idx_user_meta_quest" json:"metaQuestId"`
...
func (UserMetaQuestState) TableName() string { return "user_meta_quest_state" }
...
type QuestVersionResponse struct {
	ID            uint            `json:"id"`
	MetaQuestID   uint            `json:"metaQuestId"`
	Title         string          `json:"title"`
